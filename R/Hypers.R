#' Create hyperparameter object for betabart
#'
...
#' @export
Hypers <- function(X,Y,W,delta1, delta0, group = NULL,
                   alpha = 1,
                   beta = 2,
                   gamma = 0.95,
                   num_tree = 50,
  ...
  theta_0   <- ifelse(is.null(theta_0), qnorm(mean(delta0, na.rm = TRUE)), theta_0)
  theta_1   <- ifelse(is.null(theta_1), qnorm(mean(delta1, na.rm = TRUE)), theta_1)
  lambda_0  <- 1
  ...
  out$sigma_theta0  <- 3.0 / (k_theta * sqrt(num_tree))
  out$theta_0       <- theta_0
  out$theta_1       <- theta_1
  out$group         <- group
  out$lambda_0      <- 1
...
Opts <- function(num_burn = 2500,
  ...
  out$approximate_density <- approximate_density
