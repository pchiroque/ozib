...
struct ShapeWeight : Loglik {
  ...
  double L(double x) {
    ...
    double out = alpha * (sum_log_1mY + sum_v_Y) - n * R::lgammafn(alpha);
    // Approximated
    if (approximate)
      out += alpha * (n * log(alpha) + sum_log_v);
    else {
      arma::vec alambda = alpha * lambda;
      for (double al : alambda) out += lgamma(al + alpha) - lgamma(al);
    }

    return out;
  }

 ShapeWeight(double sumlogv, double sumlog1my, double sumvy, double nn) :
  sum_log_v(sumlogv), sum_log_1mY(sumlog1my), sum_v_Y(sumvy), n(nn),
  approximate(true) {;}
 ShapeWeight(arma::vec lambdas, double sumlog1my, double sumvy, double nn) :
   lambda(lambdas), sum_log_1mY(sumlog1my), sum_v_Y(sumvy), n(nn),
   approximate(false){;}
